...
class form{
	...
	*/
	function textarea($titleControl, $id, $name, $value, $title='', $require='0', $width='', $height='', $add_html='', $add_text='', $wysiwyg='', $add_class = ""){
		$control = $wysiwyg;
		$control .= '<textarea class="' . $this->class_form_control . ' ' . $add_class . '" title="' . $title . '" id="' . $id . '" name="' . $name . '" style="width:' . $width . 'px; height:' . $height . 'px" ' . $add_html . '>' . $this->replaceQ($value) . '</textarea>';
		$control .= $add_text;
		return $this->create_control($this->data_require($require, $titleControl, $id), $control);
	}
