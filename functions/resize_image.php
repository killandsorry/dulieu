...
function downloadImage($url_img, $save_file){
	...
	return false;
	$sExtension = getExtension($url_img);
	//Check image file type extensiton
	$checkImg = false;
	switch($sExtension){
		case "gif":
			$checkImg = @imagecreatefromgif($url_img);
			if($checkImg !== false) imagegif($checkImg, $save_file);
			break;
		case $sExtension == "jpg" || $sExtension == "jpe" || $sExtension == "jpeg":
			$checkImg = @imagecreatefromjpeg($url_img);
			if($checkImg !== false) imagejpeg($checkImg, $save_file, 100);
			break;
		case "png":
			$checkImg = @imagecreatefrompng($url_img);
			if($checkImg !== false) imagepng($checkImg, $save_file);
			break;
	}
	if($checkImg !== false){
